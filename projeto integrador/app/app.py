...
def obter_estatisticas_perfil():
    db_session = SessionLocal()
    try:
        total_respostas = db_session.query(Resposta).count()
        perfis = (
            db_session.query(Resposta.perfil_ia, func.count(Resposta.id))
            .group_by(Resposta.perfil_ia)
            .all()
        )

        contagem = {str(perfil).strip().lower(): count for perfil, count in perfis if str(perfil).strip()}
        return {
            'total': total_respostas,
            'arrojado': contagem.get('arrojado', 0),
            'conservador': contagem.get('conservador', 0),
            'moderado': contagem.get('moderado', 0),
        }
    except Exception:
        return {
            'total': 0,
...
@app.route('/dashboard')
def dashboard():
    estatisticas = obter_estatisticas_perfil()
    total = estatisticas['total']

    labels = ['Arrojado', 'Conservador', 'Moderado']
    values = [
        estatisticas['arrojado'],
        estatisticas['conservador'],
        estatisticas['moderado'],
    ]
    porcentagens = {
        'arrojado': round((estatisticas['arrojado'] / total * 100), 1) if total else 0.0,
        'conservador': round((estatisticas['conservador'] / total * 100), 1) if total else 0.0,
        'moderado': round((estatisticas['moderado'] / total * 100), 1) if total else 0.0,
    }
...
@app.route('/perfil')
def perfil():
    if not flask_session.get('usuario_id') and not flask_session.get('formulario_concluido'):
        flash('Faça login para acessar esta área', 'error')
        return redirect(url_for('login'))
...
def formulario3():
    ...
    if request.method == 'POST':
        ...
        flask_session['formulario_concluido'] = True
